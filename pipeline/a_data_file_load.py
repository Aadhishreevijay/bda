...
class FNIRSDataLoader:
    def __init__(self, root_path='/data/eeggroup/public_dataset/Tufts_fNIRS_data/'):
        ...
        # Create a mapping of available files
        self.available_files = []
        self.file_to_index = {}
        index = 1
        
        for file in sorted(os.listdir(self.root_path)):
            if file.split('.')[-1] == 'csv' and file.startswith('sub_'):
                self.available_files.append(file)
                self.file_to_index[file] = index
                index += 1
        
        self.num_subject = len(self.available_files)
        print('Total number of subjects in fNIRS database:', self.num_subject)
        print('Available files:', self.available_files[:10], '...' if len(self.available_files) > 10 else '')

    def read(self, data_index):
        assert 1 <= data_index <= self.num_subject

        # Get the actual filename for this index
        filename = self.available_files[data_index - 1]
